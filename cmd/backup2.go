...
var backupCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, _ []string) {
		...
		//TODO REFACTOR GORUTINES
		for _, storage := range storages {
			fmt.Printf("Загружаю %s в %T\n", archiveName, storage)

			fileId, err := storage.UploadFile(archiveName)
			if err != nil {
				fmt.Printf("error occured while uploading archive to %T, %v\n", storage, err)
				continue //not fail because other storages may work propely
			}

			//if some flag like scv is raised
			if rr != nil {
				if err := storeUploadedFilesValues(rr, fileId, fmt.Sprintf("%T", storage)); err != nil {
					log.Println(err)
				}
			}
		}
...
func archiveDirs(arch archiver.Archiver, srcDir, dstDir string) (string, error) {
